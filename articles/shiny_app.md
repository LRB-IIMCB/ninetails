...
|----|----|----|
| [`launch_signal_browser()`](https://LRB-IIMCB.github.io/ninetails/reference/launch_signal_browser.md) | Dorado DRS | POD5 files |
| [`launch_signal_browser_guppy()`](https://LRB-IIMCB.github.io/ninetails/reference/launch_signal_browser_guppy.md) | Guppy legacy | fast5 files |
...
The Signal Viewer tab of
[`launch_signal_browser()`](https://LRB-IIMCB.github.io/ninetails/reference/launch_signal_browser.md)
additionally requires a working Python environment with the `pod5`
package, accessible via `reticulate`. See the [installation
guide](https://github.com/LRB-IIMCB/ninetails/wiki) for details.
...
samples:
  KO_rep1:
    ...
    class_path: /path/to/KO_rep1/read_classes.txt
    residue_path: /path/to/KO_rep1/nonadenosine_residues.txt
    nanopolish_path: /path/to/KO_rep1/nanopolish_output.tsv   # optional
    sequencing_summary_path: /path/to/KO_rep1/sequencing_summary.txt  # optional
...
| `class_path` | Yes | Path to `read_classes.txt` from ninetails |
| `residue_path` | Yes | Path to `nonadenosine_residues.txt` from ninetails |
| `nanopolish_path` | No | Path to Nanopolish polya output (for Signal Viewer) |
| `sequencing_summary_path` | No | Path to Guppy sequencing summary (for Signal Viewer) |
| `workspace` | No | Path to multi-fast5 directory (for Signal Viewer) |

### Guppy-specific dashboard features

The Guppy dashboard provides the same six tabs as the Dorado version,
with two additions:
...
instead of POD5-based functions. Includes a **Moves** toggle to show or
hide basecaller move transitions in the signal background. No Python
...
Each residue type is subsampled to a maximum of 1,000 points per sample.
...
Controls include grouping, transcript filter, condition selection,
central tendency overlay, x-axis limit, normalization, and color
palette.
...
`nanopolish_file`, `sequencing_summary_file`, and `workspace`. Includes
a **Moves** toggle to show basecaller move transitions.

Both versions provide:

- *Static Viewer* — two ggplot2 plots (entire signal and poly(A) region
  with non-A modification overlays)
- *Dynamic Explorer* — interactive Plotly chart with zoom, pan, and
  hover
...
if it contains `workspace` or `nanopolish`, the Guppy app is used.
...
`base64enc`. Install with `install.packages("base64enc")`.

**Signal Viewer shows “No POD5 file found”** *(Dorado)* Ensure
`pod5_dir` points to the directory containing `.pod5` files. The app
searches recursively within that directory.

**Signal Viewer shows “Extraction failed”** *(Dorado)* The Python `pod5`
package must be installed and accessible via `reticulate`. Check with:
...
**Signal Viewer shows no signal** *(Guppy)* Ensure `nanopolish_file`,
`sequencing_summary_file`, and `workspace` all point to valid paths
accessible to the R session. The read ID selected must exist in the
Nanopolish output.

**Classification or Residue plots are blank** In single-sample mode,
`class_file` and/or `residue_file` must be provided. Without them, the
