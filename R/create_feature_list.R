#' Extracts features of RNA reads required for finding non-A nucleotides
#' in polyA tails.
#'
#' This function extracts features of RNA reads from multi-fast5 files
...
#'\dontrun{
#'
#' create_feature_list(nanopolish = '/path/to/file',
...
create_feature_list <- function(nanopolish, sequencing_summary, workspace, num_cores, basecall_group, pass_only=TRUE){
  ...
  for (indx in 1:length(index_list)){
    ...
    extracted_data_multiple_list <- c(extracted_data_multiple_list, foreach::foreach(nam = names(polya_summary$filename)[index_list[[indx]]]) %dopar% extract_squiggle_data(nam,polya_summary,workspace,basecall_group))
