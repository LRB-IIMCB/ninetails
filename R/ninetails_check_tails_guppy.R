  ...
  tryCatch(
    {
      ...
      cli_log(
        "Launching *check_tails_guppy* pipeline",
        "INFO",
        "Launching *check_tails_guppy* pipeline"
      )
      ...
      cli_log(
        sprintf(
          "Poly(A) length file:         %s",
          if (!is.object(polya_data)) {
            polya_data
          } else {
            deparse(substitute(polya_data))
          }
        ),
        bullet = TRUE
      )
      cli_log(
        sprintf(
          "Sequencing summary:          %s",
          if (!is.object(sequencing_summary)) {
            sequencing_summary
          } else {
            deparse(substitute(sequencing_summary))
          }
        ),
        bullet = TRUE
      )
      cli_log(
        sprintf("Fast5 files directory:       %s", workspace),
        bullet = TRUE
      )
      cli_log(
      ...
      cli_log(
        sprintf("Basecall group:              %s", basecall_group),
        bullet = TRUE
      )
      cli_log(
        sprintf("Only PASS reads:             %s", pass_only),
        bullet = TRUE
      )
      ...
      assert_condition(
        is_string(basecall_group, min.chars = 1),
      ...
      if (is_string(polya_data)) {
        ...
        assert_condition(
          is.data.frame(polya_data) && nrow(polya_data) > 0,
      ...
      if (is_string(sequencing_summary)) {
        ...
        assert_condition(
          is.data.frame(sequencing_summary) && nrow(sequencing_summary) > 0,
      ...
      cli_log(
        "Checking FAST5 format...",
        "INFO",
        "Validating FAST5 Data",
        bullet = TRUE
      )
      ...
      cli_log(
        "Checking poly(A) data input size...",
        "INFO",
        "Poly(A) Input Size Check",
        bullet = TRUE
      )
      ...
      if (data_size > part_size) {
        cli_log(
          sprintf(
            "Large input detected (%d rows). Processing in parts of %d rows...",
            data_size,
            part_size
          ),
          "INFO"
        ...
        # Split input
        part_files <- ninetails::split_polya_data(
          polya_data,
          part_size,
          save_dir
        )
        cli_log(
          sprintf("Input split into %d parts", length(part_files)),
          "INFO"
        )
        ...
        cli_log(
          sprintf(
            "Input size (%d rows) within limits. Processing as single file.",
            data_size
          ),
          "INFO"
        )
...
#'
#' @export
process_polya_complete <- function(
  polya_data,
  sequencing_summary,
  workspace,
  num_cores,
  ...
  cli_log(
    "Checking poly(A) length file format...",
    "INFO",
    "Processing Poly(A) length data",
    bullet = TRUE
  )
  ...
  polya_data <- tryCatch(
    {
      ...
      if (result$file_type == "nanopolish") {
        ...
        cli_log(
          "Tailfindr DRS output detected. Converting to nanopolish-compatible format.",
          "WARNING"
        )
        cli_log(
          "NOTE: Tailfindr does not provide quality metrics for poly(A) tail calling.",
          "WARNING"
        )
        cli_log("Results should be interpreted with caution.", "WARNING")
        cli_log(
          "Quality tags are assigned based on tail length only (PASS for >= 10nt, NOREGION for < 10nt).",
          "WARNING"
        )
      }
    ...
    error = function(e) {
      cli_log(
        sprintf("Error in poly(A) input validation: %s", e$message),
        "ERROR"
      )
      stop(e$message, call. = FALSE)
    }
  ...
  tail_feature_list <- tryCatch(
    {
      ...
      if (length(features$tail_feature_list) == 0) {
        stop(
          "Produced feature list is of length 0. Check your input (nanopolish, fast5 files) for integrity.",
          call. = FALSE
        )
      }
...
process_polya_parts <- function(
  ...
  cli_log(
    sprintf(
      "Starting sequential processing of %d poly(A) data parts",
      num_parts
    ),
    "INFO",
    "Poly(A) Parts Processing",
    bullet = TRUE
  )
  ...
  for (i in seq_along(part_files)) {
    ...
    cli_log(
      sprintf("Processing part %d of %d", i, num_parts),
      "INFO",
      sprintf("Part %d Processing", i),
      bullet = TRUE
    )
    ...
    cli_log(
      sprintf("Poly(A) input file part %d processed successfully", i),
      "SUCCESS"
    )
    cli_log(
      sprintf("Poly(A) input file part %d results saved in: %s", i, part_dir),
      "INFO"
    )
  }
  ...
  cli_log(
    "Merging results from all Poly(A) input file parts...",
    "INFO",
    "Final Processing"
  )
  ...
  cli_log(
    sprintf("Total reads processed: %d", nrow(merged_output$read_classes)),
    "INFO"
  )
  cli_log(
    sprintf(
      "Total residues identified: %d",
      nrow(merged_output$nonadenosine_residues)
    ),
    "INFO"
  )
